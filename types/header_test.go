...
func TestDecodeParachainHeader(t *testing.T) {
	...
	require.Equal(t, rpcclienttypes.BlockNumber(rpcclienttypes.NewU32(14)), header.Number, "failed to check block number from decoded header")
...
func TestHeader(t *testing.T) {
	h := rpcclienttypes.Header{
		Digest: rpcclienttypes.Digest{
			rpcclienttypes.DigestItem{
				AsConsensus: rpcclienttypes.Consensus{
					//ConsensusEngineID: rpcclienttypes.NewBytes([]byte{1, 2, 3}),
					Bytes: rpcclienttypes.NewBytes([]byte("/IBC")),
				},
			},
		},
	}
	...
	require.Equal(t, rpcclienttypes.NewBytes([]byte("/IBC")), h.Digest[0].AsConsensus.Bytes)
